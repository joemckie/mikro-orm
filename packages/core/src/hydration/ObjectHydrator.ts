  ...
  getEntityHydrator<T extends object>(meta: EntityMetadata<T>, type: 'full' | 'reference', normalizeAccessors = false): EntityHydrator<T> {
    ...
    const hydrateToOne = (prop: EntityProperty, dataKey: string, entityKey: string) => {
      ...
      if (prop.polymorphic) {
        ...
        ret.push(`      } else {`);
        ret.push(`        throw new Error(\`Unknown discriminator value '\${data${dataKey}.discriminator}' for polymorphic relation '${prop.name}'. Valid values: \${Object.keys(${discriminatorMapKey}).join(', ')}\`);`);
        ret.push(`      }`);
        ret.push(`    }`);
        ret.push(`  }`);
