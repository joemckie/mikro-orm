...
export class MetadataDiscovery {
  ...
  private mapDiscoveredEntities(): MetadataStorage {
    ...
    for (const meta of discovered) {
      ...
      // Compute TPT ownProps on final metadata
  ...
  processDiscoveredEntities(discovered: EntityMetadata[]): EntityMetadata[] {
    ...
    return discovered.map(meta => {
      ...
      // Final TPT ownProps computation on the registry metadata with final props
      if (meta.inheritanceType === 'tpt') {
        this.computeTPTOwnProps(meta);
      }
    ...
    if (!computeOwnProps) {
      ...
      if (meta.root !== meta) {
        ...
        if (parent) {
          ...
          if (!meta.tableName && meta.name) {
            meta.tableName = this.namingStrategy.classToTableName(meta.name);
          }
        }
      }
      return;
  ...
  private initTPTDiscriminator(meta: EntityMetadata, metadata: EntityMetadata[]): void {
    ...
    const discriminatorColumn = '__tpt_discriminator';
  ...
  private computeTPTOwnProps(meta: EntityMetadata): void {
    // Filter out virtual properties (like __tpt_discriminator) which are not persisted
    const isPersisted = (prop: EntityProperty) => prop.persist !== false || prop.primary;

    if (!meta.tptParent) {
      meta.ownProps = meta.props.filter(isPersisted);
    } else {
      const parentPropNames = new Set(meta.tptParent.props.map(p => p.name));
      meta.ownProps = meta.props.filter(prop => !parentPropNames.has(prop.name) && isPersisted(prop));
  ...
  private getTPTParent(meta: EntityMetadata, metadata: EntityMetadata[]): EntityMetadata | undefined {
    ...
    const parent = metadata.find(m => {
      ...
      return m.className === Utils.className(ext as EntityName);
    });
