...
describe('custom types [mysql]', () => {
  ...
  test('advanced custom types', async () => {
    ...
    const res2 = await qb2.select(['l0.*'] as any).where({ id: loc.id }).getSingleResult();
    ...
    expect(mock.mock.calls[0][0]).toMatch('select `a0`.*, `l1`.`id` as `l1__id`, `l1`.`rank` as `l1__rank`, ST_AsText(`l1`.`point`) as `l1__point`, ST_AsText(`l1`.`extended_point`) as `l1__extended_point` from `address` as `a0` inner join `location` as `l1` on `a0`.`location_id` = `l1`.`id` where `a0`.`id` = 1');
