...
> Support for TPT was added in version 7.0
...
The current TPT implementation has the following limitations:

- **Polymorphic queries**: Querying the base class to get mixed concrete types is not yet supported. You must query specific concrete entity types.
- **Relations to abstract TPT entities**: Relations that reference an abstract TPT base class require the concrete entities to be persisted first.
